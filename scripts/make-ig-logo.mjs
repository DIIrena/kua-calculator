// Renders the brand heart-house mark as square social avatars
// (1080x1080 PNG) in two brand colourways. JPG conversion happens in
// PowerShell afterwards (System.Drawing). Output: brand/.
//
// Usage: node scripts/make-ig-logo.mjs

import { Resvg } from "@resvg/resvg-js";
import { writeFileSync, mkdirSync } from "node:fs";
import path from "node:path";

const OUT_DIR = path.join(process.cwd(), "brand");
mkdirSync(OUT_DIR, { recursive: true });

// The BrandHeart paths (components/BrandHeart.tsx), verbatim.
const HEART_PATHS = `
...
const VARIANTS = [
  { name: "ig-avatar-ivory-on-green", bg: "#0e3b2c", fg: "#fcfcf8" },
  { name: "ig-avatar-green-on-ivory", bg: "#fcfcf8", fg: "#0e3b2c" },
];

for (const v of VARIANTS) {
  const png = new Resvg(avatarSvg(v.bg, v.fg), {
