/* ============================================================
   cny.js
   Chinese New Year (lunar new year) dates 1900-2050, Gregorian.
   Used to determine the effective Kua year: someone born before
   Chinese New Year belongs to the previous Chinese solar-calendar
   year for Kua purposes, per methodology.md section 5.
   ============================================================ */

(function (root) {
  "use strict";

  // year -> "MM-DD" of Chinese New Year in Gregorian calendar
  ...
  /**
   * Return the Chinese solar-calendar year for a Gregorian birth date.
   * Most births: returns the same year, adjusted=false.
   * Births before Chinese New Year of their Gregorian year: returns
   * year - 1, adjusted=true, with the CNY date that triggered the shift.
   * Years outside the table: returns the input year, adjusted=null
   * (caller should warn the user we can't auto-adjust).
   */
  function effectiveKuaYear(year, month, day) {
    ...
    var parts = cny.split("-");
    var cnyMonth = parseInt(parts[0], 10);
    var cnyDay = parseInt(parts[1], 10);

    var beforeCny = (month < cnyMonth) || (month === cnyMonth && day < cnyDay);
    if (beforeCny) {
      return {
        year: year - 1,
        adjusted: true,
    ...
    return { year: year, adjusted: false };
  }

  root.effectiveKuaYear = effectiveKuaYear;
  root.MONTH_NAME = MONTH_NAME;
