...
import { findGuidePage, findCluster } from "@/lib/guide";

// Per-guide-page OG image. 1200x630 PNG, brand-styled, generated on-demand.
// Mirrors the article OG route at app/api/og/[slug]/route.ts; reuses the
// same bundled Hanken Grotesk TTFs and SVG layout, swapping in guide data.
...
function fontPath(file: string): string | null {
  ...
  try {
    ...
    console.error("[og:guide] font missing", file, err);
    return null;
  }
}
...
function buildSvg(args: { title: string; clusterLabel: string }): string {
  const W = 1200;
  const H = 630;
  const PAD_X = 80;
  ...
  const eyebrow = args.clusterLabel.toUpperCase();
  ...
  <text x="${PAD_X}" y="${H - 64}" font-family="Hanken Grotesk" font-size="26" font-weight="500" fill="${COLOR.ink2}">The Ultimate Feng Shui Guide</text>
...
export async function GET(
  ...
  context: { params: Promise<{ cluster: string; slug: string }> },
) {
  const { cluster, slug } = await context.params;
  ...
  const svg = buildSvg({ title: page.title, clusterLabel: clusterMeta.label });
